 ...
 * Using digitalWrite() and __delay_us() we will have frequent 8-12us timing errors
...
void send(byte* data, uint8_t len, boolean incrementSeq){
	...
	TRF_TX_PIN =  LOW;
	__delay_us(START_PULSE_PERIOD - PERIOD_HIGH_DURATION);
	TRF_TX_PIN = HIGH;
	__delay_us(PERIOD_HIGH_DURATION);
	...
	for(uint8_t i=0; i<10; i++){
		__delay_us(PERIOD_HIGH_DURATION/2);
		TRF_TX_PIN = HIGH;
		__delay_us(PERIOD_HIGH_DURATION/2);
		TRF_TX_PIN = LOW;
	}
#else
	//__delay_us(MIN_TX_INTERVAL_REAL*2);
#endif
...
void sendMulti(byte data[], uint8_t len, uint8_t times){
	for(uint8_t i=0; i<times; i++){
		...
		__delay_us(MIN_TX_INTERVAL_REAL + TX_INTERVAL_CONST);
	}
	#ifndef TRF_SEQ_DISABLED
	seq++;
	#endif
...
void transmitByte(byte _byte){
	...
	do{
		...
		//if 1
		if(_byte & (1<<i)){
			__delay_us(ONE_PULSE_PERIOD - PERIOD_HIGH_DURATION);
		}
		else{
			__delay_us(ZERO_PULSE_PERIOD - PERIOD_HIGH_DURATION);
		}
